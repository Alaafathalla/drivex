...
export default function RentalDetailsPage({ params }) {
  ...
  const handleBook = () => {
    if (!pickup || !returnDate || returnDate <= pickup) { setError(t('rent_choose_valid_dates')); return }
    if (!location) { setError(t('rent_choose_pickup_loc')); return }
    if (!car?.available) { setError(t('rent_car_unavailable_toast')); return }
    setError('')
    const booking = {
      pickupDate: pickup,
      returnDate,
  ...
  const specs = [[Settings2, car.transmission], [Users, `${car.seats} ${t('rent_seats_label')}`], [Fuel, car.fuel], [Gauge, t('rent_unlimited_km')]]

  return (
    <main className="min-h-screen bg-[#F5F6F3]" dir={isRTL ? 'rtl' : 'ltr'}>
      <section className="page-inner py-10 sm:py-14">
        <a href="/rentals" className="mb-6 inline-flex text-xs font-black uppercase tracking-[.12em] text-[#64748B] hover:text-[#0F172A]">← {t('rent_back_to_rentals')}</a>
        <div className="grid gap-8 lg:grid-cols-[1.35fr_.75fr]">
          <div>
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative aspect-[1.62] overflow-hidden rounded-[28px] bg-[#111]">
              ...
              <span className={`absolute left-4 top-4 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[.12em] ${car.available ? 'bg-[#B5E92E] text-[#091219]' : 'bg-white text-[#091219]'}`}>{car.available ? t('rent_available') : t('rent_unavailable')}</span>
            </motion.div>
            ...
            <div className="mt-8">
              <p className="text-xs font-black uppercase tracking-[.18em] text-[#7C8B55]">{t('rent_page_eyebrow')}</p>
          ...
          <motion.aside initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="h-fit rounded-[28px] border border-[#E2E6DE] bg-white p-6 shadow-[0_24px_60px_rgba(15,23,42,.08)] lg:sticky lg:top-24">
            <div className="flex items-end justify-between"><div><p className="text-xs font-bold text-[#94A3B8]">{t('rent_daily_rate')}</p><p className="mt-1 text-4xl font-black tracking-[-.04em] text-[#0F172A]">{format(car.pricePerDay)}<span className="text-xs font-bold text-[#94A3B8]"> / {t('rent_per_day')}</span></p></div><Sparkles className="text-[#7C8B55]" size={20} /></div>
