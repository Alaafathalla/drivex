...
function monthGrid(monthDate) {
  ...
  return Array.from({ length: 42 }, (_, index) => {
    ...
    if (dayNumber < 1) {
      return {
        date: new Date(year, month - 1, previousDays + dayNumber),
        current: false
      }
    }
    ...
    if (dayNumber > days) {
      return {
        date: new Date(year, month + 1, dayNumber - days),
        current: false
      }
    }
...
export function RentalDateRangePicker({
  ...
  const initial =
    new Date(
      `${start || minDate || iso(new Date())}T12:00:00`
    )
...
shadow-[0_30px_80px_rgba(15,23,42,.24)]
`}
...
selectedStart
?
'rounded-l-xl bg-[#0e1418] text-white'
:
selectedEnd
?
'rounded-r-xl bg-[#0e1418] text-white'
:
inRange
?
'bg-[#B5E92E]/20 text-[#536a14]'
:
current
?
'text-slate-700 hover:bg-[#B5E92E]/20'
:
'text-slate-300'
}
