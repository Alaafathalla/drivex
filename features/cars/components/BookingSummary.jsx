  ...
  return (
    <div className={`overflow-hidden rounded-2xl border border-gray-200 bg-white ${compact ? '' : 'shadow-sm'}`}>
      ...
      <div className="space-y-2.5 p-4">
        <div className="flex items-start gap-2.5"><CalendarDays size={15} className="mt-0.5 shrink-0 text-[#22c55e]" /><div><p className="text-[11px] text-gray-400">{t('booking_pickup')}</p><p className="text-[13px] font-semibold text-gray-900">{booking.pickupDate} {booking.pickupTime && `· ${booking.pickupTime}`}</p><p className="text-[11px] text-gray-500">{booking.pickupLocation}</p></div></div>
        <div className="h-px bg-gray-100" />
        <div className="flex items-start gap-2.5"><CalendarDays size={15} className="mt-0.5 shrink-0 text-rose-400" /><div><p className="text-[11px] text-gray-400">{t('booking_return')}</p><p className="text-[13px] font-semibold text-gray-900">{booking.returnDate} {booking.returnTime && `· ${booking.returnTime}`}</p><p className="text-[11px] text-gray-500">{booking.dropoffLocation}</p></div></div>
      </div>
      {booking.total && <div className="flex items-center justify-between border-t border-gray-100 bg-[#f0fdf4] px-4 py-3"><p className="text-[12px] text-gray-500">{t('booking_total_label')} ({booking.days} {t('booking_days')})</p><p className="text-[18px] font-black text-[#16a34a]">${booking.total}</p></div>}
    </div>
  )
}
