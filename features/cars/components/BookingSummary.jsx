  ...
  return (
    <div className={`overflow-hidden rounded-2xl border border-gray-200 bg-white ${compact ? '' : 'shadow-sm'}`}>
      ...
      <div className="space-y-2.5 p-4">
        <div className="flex items-start gap-2.5"><CalendarDays size={15} className="mt-0.5 shrink-0 text-[#22c55e]" /><div><p className="text-[11px] text-gray-400">Pickup</p><p className="text-[13px] font-semibold text-gray-900">{booking.pickupDate} {booking.pickupTime && `· ${booking.pickupTime}`}</p><p className="text-[11px] text-gray-500">{booking.pickupLocation}</p></div></div>
      ...
      {booking.total && <div className="flex items-center justify-between border-t border-gray-100 bg-[#f0fdf4] px-4 py-3"><p className="text-[12px] text-gray-500">Total ({booking.days} days)</p><p className="text-[18px] font-black text-[#16a34a]">${booking.total}</p></div>}
    </div>
  )
}
